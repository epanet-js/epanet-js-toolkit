...
const TARGET_D_TS_PATH = path.join(
  path.dirname(import.meta.url.substring(7)),
  "../dist/index.d.ts",
); // Where we want to write the .d.ts
...
const KNOWN_POINTER_TYPES = [
  ...
  "EN_Project",
  "void *",
]; // Add known pointer types (EN_Project is likely a typedef for a pointer)
  ...
  switch (cleanCType) {
    ...
    default:
      ...
      if (cleanCType.startsWith("EN_") || cleanCType.startsWith("struct")) {
        console.warn(
          ` - Assuming pointer type for unknown C type: ${cleanCType}.`,
        );
        return POINTER_TYPE_NAME;
      }
      console.warn(
        ` - Unhandled C type encountered: ${cleanCType}. Defaulting to 'any'.`,
      );
...
function parseParametersString(paramString) {
  ...
  for (const part of correctlySplitParts) {
    ...
    const closingParenIndex =
      starParenIndex !== -1 ? trimmedPart.indexOf(")", starParenIndex + 2) : -1;

    if (starParenIndex !== -1 && closingParenIndex !== -1) {
      // Likely a function pointer. Extract name between '(*' and ')'
      paramName = trimmedPart
        .substring(starParenIndex + 2, closingParenIndex)
        .trim();
      ...
      if (trimmedPart.indexOf("(", closingParenIndex) === -1) {
        console.warn(
          ` - Potential function pointer parse issue (missing args parenthesis?): ${trimmedPart}`,
        );
      }
      console.log(
        `INFO: Detected function pointer: Name='${paramName}', FullType='${paramType}'`,
      );
    } else {
      ...
      if (lastSpaceIndex === -1) {
        ...
        if (trimmedPart.length > 0) {
          ...
          console.warn(
            ` - Could not parse parameter part (no space?): '${trimmedPart}'. Assuming type only, using placeholder name '${paramName}'.`,
          );
        } else {
        ...
        if (
          !typeNeedsPointer &&
          trimmedPart.substring(lastSpaceIndex + 1).includes("*") &&
          !paramName.includes("*")
        ) {
          typeNeedsPointer = true;
        }

        if (typeNeedsPointer && !paramType.endsWith("*")) {
          paramType += " *";
        }
    ...
    if (!paramName || !paramType) {
      console.warn(
        ` - Failed to extract type or name for parameter part: ${trimmedPart}`,
      );
      continue;
    }
  ...
  for (const rawLine of jsdocLines) {
    ...
    if (!startsWithTag && currentTag && cleanLine) {
      ...
      if (currentTag === "brief") {
        ...
        currentTag === "param" &&
        currentParamName &&
        data.params[currentParamName]
      ) {
        // Append to the tracked parameter's description
        data.params[currentParamName].description +=
          (data.params[currentParamName].description ? appendSeparator : "") +
          cleanLine;
      }
    }
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (line.startsWith("/**")) {
    ...
    if (inJSDoc) {
      failures.push(
        `Line ${i + 1}: New JSDoc started before previous one ended.`,
      );
    }
    inJSDoc = true;
  ...
  if (!inJSDoc && currentJSDocLines.length > 0 && line) {
    ...
    while (!foundSemicolon && signatureEndLine + 1 < lines.length) {
      ...
      if (nextLine.startsWith("/**") || nextLine.startsWith("#")) {
        failures.push(
          `Line ${i + 1}-${signatureEndLine}: Signature potentially interrupted by comment/directive before semicolon.`,
        );
      ...
      if (signatureEndLine > i + 10) {
        // Arbitrary limit
        failures.push(
          `Line ${i + 1}-${signatureEndLine + 1}: Potential runaway function signature parsing for: ${line}`,
        );
    ...
    if (!foundSemicolon) {
      failures.push(
        `Line ${i + 1}-${signatureEndLine + 1}: Could not find terminating semicolon for potential function signature starting with: ${line}`,
      );
    ...
    if (
      ...
      semicolonIndex < parenCloseIndex
    ) {
      failures.push(
        `Line ${i + 1}-${signatureEndLine + 1}: Malformed signature structure (parentheses/semicolon): ${signature}`,
      );
      parseFailed = true;
    } else {
      const beforeParams = signature.substring(0, parenOpenIndex).trim();
      const paramString = signature
        .substring(parenOpenIndex + 1, parenCloseIndex)
        .trim();
      ...
      if (!functionName || !returnType) {
        failures.push(
          `Line ${i + 1}-${signatureEndLine + 1}: Could not extract function name or return type from: ${beforeParams}. Signature: ${signature}`,
        );
        ...
        try {
          ...
          failures.push(
            `Line ${i + 1}-${signatureEndLine + 1}: Error processing params/JSDoc for ${functionName}: ${e.message}. Signature: ${signature}`,
          );
          parseFailed = true; // Mark as failed
        }
      }
...
const tsFunctionDefs = functions
  .map((f) => {
    ...
    if (f.returnDescription)
      tsComment += `     * @returns ${f.returnDescription}\n`;
...
const boilerplateBottom = `
    ...
    // --- OPFS Helper Functions (available in pthreads build only) ---
    /**
     * Check if this build was compiled with OPFS support.
     * Returns 1 for the pthreads/WasmFS build, undefined/missing for standard build.
     */
    _epanet_supports_opfs?(): number;

    /**
     * Check at runtime whether the browser environment supports OPFS.
     * Returns 1 if OPFS is available, 0 otherwise.
     */
    _epanet_can_mount_opfs?(): number;

    /**
     * Mount OPFS at the specified path using WasmFS OPFS backend.
     * @param mount_path Pointer to a C string with the mount path (e.g., "/opfs")
     * @returns 0 on success, -1 on failure
     */
    _epanet_mount_opfs?(mount_path: ${POINTER_TYPE_NAME}): number;
}
...
console.log(
  `Successfully generated ${TEMP_D_TS_PATH} with ${functions.length} functions.`,
);
...
try {
  ...
  console.warn(
    `Warning: Could not move file to ${TARGET_D_TS_PATH}. The file remains at ${TEMP_D_TS_PATH}`,
  );
